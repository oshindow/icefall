...
def greedy_search_batch(
    ...
    assert encoder_out.ndim == 3
    assert encoder_out.size(0) >= 1, encoder_out.size(0)
    # print(encoder_out_lens.shape, encoder_out.shape)
    ...
    assert N == batch_size_list[0], (N, batch_size_list[0])
...
def greedy_search_pitch_batch(
    ...
    decoder_out = model.decoder(decoder_input, need_pad=False)
    pitch_out = model.pitch_decoder(pitch)
    if decoder_out.size(1) <= pitch_out[:,::4,:].size(1):
        pitch_out = pitch_out[:,::4,:][:,:decoder_out.size(1),:]
        
    else:
        scaled_length = pitch_out[:,::4,:].size(1)
        pitch_out = torch.nn.functional.pad(pitch_out[:,::4,:],pad=(0,0,0,decoder_out.size(1) - scaled_length), mode='reflect')
          
    decoder_out = model.joiner.decoder_proj(decoder_out)
    # decoder_out: (N, 1, decoder_out_dim)

    encoder_out = model.joiner.encoder_proj(packed_encoder_out.data)
    pitch_out = model.joiner.pitch_proj(pitch_out)
    ...
    for t, batch_size in enumerate(batch_size_list):
        ...
        current_encoder_out = current_encoder_out.unsqueeze(1).unsqueeze(1)
        # current_encoder_out's shape: (batch_size, 1, 1, encoder_out_dim)
        current_pitch_out = pitch_out.data[start:end]
        current_pitch_out = current_pitch_out.unsqueeze(1).unsqueeze(1)
        offset = end

        decoder_out = decoder_out[:batch_size]
        
        logits = model.joiner(
            current_encoder_out, decoder_out.unsqueeze(1), current_pitch_out, project_input=False
        )
        # logits'shape (batch_size, 1, 1, vocab_size)
