...
from finetune_hubert_transducer.train_phone_model_w2v_ctc import get_params, get_transducer_model
...
def decode_one_batch(
    ...
    # print(ctc_output.shape, encoder_out_lens)
    lens = torch.ones(ctc_output.size(0)).to(ctc_output.device)
    sequences = ctc_greedy_decode(ctc_output, lens, blank_id=0)
    # print(sequences)
    predicted_words_list = []
    for sequence in sequences:
        # Decode token terms to words
        predicted_tokens = [lexicon.token_table[phone_id] for phone_id in sequence]
        # print(predicted_tokens)
        predicted_words = []
        for c in predicted_tokens:
            if c == "SIL":  # ""[CLS]":
                continue
            elif c == "SPN": # "[SEP]" or c == "[PAD]":
                break
            else:
...
def main():
    ...
    wav2vecEncoder.add_arguments(parser)
