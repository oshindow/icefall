...
from finetune_hubert_transducer.train_phone_model_f0_emb_conv_att_mel_coarse import get_params, get_transducer_model
...
def decode_one_batch(
    ...
    # f0_mel = 1127 * torch.log(1 + f0 / 700).long()
    f0 = f0.cpu().numpy()
    ...
    f0_coarse = torch.LongTensor(f0_coarse).to(device)
    ...
    f0 = model.pitch_emb(f0_coarse.squeeze(-1))
    if f0.size(1) % 4 != 0:
        pad_length = 4 - f0.size(1) % 4 
        f0 = F.pad(f0, pad=(0,0,0,pad_length))

    f0 = f0.transpose(1, 2)
    h = model.f0_encoder(f0) 
