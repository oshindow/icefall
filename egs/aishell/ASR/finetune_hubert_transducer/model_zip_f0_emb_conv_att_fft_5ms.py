...
class AsrModel(nn.Module):
    def __init__(
        ...
        self.f0_conv1 = torch.nn.Sequential(
            torch.nn.Conv1d(256, 512, 3, 2, 1), 
            torch.nn.GroupNorm(8, 512), 
            Mish())
        
        self.f0_fft1 = FFTBlock(d_model=512, d_inner=512, n_head=4, d_k=512, d_v=512, l_n=2, cln_ffd=False, dropout=0.1)
        ...
        self.f0_fft2 = FFTBlock(d_model=512, d_inner=1024, n_head=4, d_k=512, d_v=512, l_n=2, cln_ffd=False, dropout=0.1)
        
        self.f0_conv3 = torch.nn.Sequential(
            torch.nn.Conv1d(512, 1024, 3, 2, 1), 
            torch.nn.GroupNorm(8, 1024), 
            Mish())
        
        self.f0_fft3 = FFTBlock(d_model=1024, d_inner=1024, n_head=4, d_k=1024, d_v=1024, l_n=2, cln_ffd=False, dropout=0.1)
        ...
        self.pitch_emb = nn.Embedding(
            self.pitch_bin_size, 256, padding_idx=0
        )
        
        self.res_conv = torch.nn.Conv1d(256, 1024, 17, 8, 7)
    ...
    def forward(
        ...
        # f0 encoder conv subsampling 2 
        h = self.f0_conv2(h)
        subsampled_length = subsampled_length // 2 + 1
        f0_mask = f0_mask[:,::2,:][:, :subsampled_length,:]

        # f0 encoder transformer 2
        h = h.transpose(1, 2)
        h = self.f0_fft2(h, non_pad_mask=f0_mask, slf_attn_mask=None)
        h = Mish()(h)
        h = h.transpose(1, 2)

        # f0 encoder conv subsampling 3
        h = self.f0_conv3(h)
        subsampled_length = subsampled_length // 2 + 1
        f0_mask = f0_mask[:,::2,:][:, :subsampled_length,:]

        # f0 encoder transformer 3
        h = h.transpose(1, 2)
        h = self.f0_fft3(h, non_pad_mask=f0_mask, slf_attn_mask=None)
        h = Mish()(h)
        h = h.transpose(1, 2)
        ...
        # print(encoder_out.shape, f0_emb.shape)
