...
from cif import CifPredictorV2
...
class GradLogPEstimator2d(BaseModule):
    def __init__(self, dim, dim_mults=(1, 2, 4), groups=4,
        ...
        self.pitch_emb = torch.nn.Embedding(256, 1024)
    ...
    def forward(self, input, condition=None):
        ...
        # print(input.shape, x.shape, condition.shape)
        x = x.unsqueeze(1) + condition.unsqueeze(1)

        # cif
        pre_token, token_num, alphas, cif_peak = self.cif(x.squeeze(1))
        # print(pre_token.shape, x.shape)
        x = pre_token
        # strict x size
        if x.size(1) % 4 != 0:
            pad_length = 4 - x.size(1) % 4 
            x = F.pad(x, pad=(0,0,0,pad_length))
        x = x.unsqueeze(1)
        ...
        return output.squeeze(1), pre_token, token_num, alphas, cif_peak
