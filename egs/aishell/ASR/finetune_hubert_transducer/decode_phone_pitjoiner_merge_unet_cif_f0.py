...
from finetune_hubert_transducer.train_phone_model_pitjoiner_merge_unet_cif_mae_f0 import get_params, get_transducer_model
...
def decode_one_batch(
    ...
    # encoder_out, token_num, alphas, cif_peak = model.unet(input=pitch, condition=encoder_out)
    encoder_out, pre_token, token_num, alphas, cif_peak = model.unet(input=pitch, condition=encoder_out)
    hyps = []
    ...
    if params.decoding_method == "fast_beam_search":
        ...
        hyp_tokens, y = greedy_search_batch(
            model=model,
            encoder_out=encoder_out,
            encoder_out_lens=encoder_out_lens,
        )
        # for hyp in sp.decode(hyp_tokens):
        #     hyps.append(hyp.split())
        print(pre_token.shape)
        for i in range(encoder_out.size(0)):
            hyps.append([lexicon.token_table[idx] for idx in hyp_tokens[i]])
            print('y:',y[i],'hyps:', hyps[i], 'cif_peak:', cif_peak[i], 'token_num:', token_num[i], 'alphas:', alphas[i], 'encoder_out:', encoder_out[i].shape)
