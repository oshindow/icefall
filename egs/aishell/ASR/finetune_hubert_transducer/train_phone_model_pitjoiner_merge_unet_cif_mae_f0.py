...
from model_zip_pitjoiner_unet_cif_mae_f0 import AsrModel
...
def compute_loss(
    ...
    pitch = batch['customs']['pitch'].long()
    f0 = batch['customs']['f0'].long()
    ...
    with torch.set_grad_enabled(is_training):
        simple_loss, pruned_loss,_, loss_pre,loss_f0 = model(
            x=feature,
            pitch=pitch,
            f0=f0,
            x_lens=feature_lens,
        ...
        loss = params.simple_loss_scale * simple_loss + pruned_loss_scale * pruned_loss + loss_pre + loss_f0
        # loss = params.simple_loss_scale * simple_loss + pruned_loss_scale * pruned_loss + loss_pre
    ...
    info["loss_pre"] = loss_pre.detach().cpu().item()
    info["loss_f0"] = loss_f0.detach().cpu().item()
