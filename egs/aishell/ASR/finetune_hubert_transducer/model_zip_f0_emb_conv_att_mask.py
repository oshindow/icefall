...
class F0_encoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        mask = mask[:,::2][:, :subsampled_length]
        # print('after block1 f0:', x.shape, mask.shape)
        
        x = x.masked_fill(mask.unsqueeze(1).expand_as(x), 0.0)
        # print('after block1 f0:', x)

        x = self.block2(x)
        subsampled_length = subsampled_length // 2 + 1
        mask = mask[:,::2][:, :subsampled_length]
        # print('after block2 f0:', x.shape, mask.shape)
        x = x.masked_fill(mask.unsqueeze(1).expand_as(x), 0.0)
        # print('after block2 f0:', x)
        return x, mask
...
class AsrModel(nn.Module):
    def __init__(
        ...
        self.res_conv = torch.nn.Conv1d(512, 1024, 5, 4, 1)
        
        # f0 joiner
        self.output_linear = nn.Linear(1024, 1024)
    ...
    def forward(
        ...
        # convert original f0_lens into int for padding
        f0_mask = make_pad_mask(f0_lens)
        f0_mask = f0_mask.to(torch.int)

        # print('int mask:', f0_mask)
        # print('int mask:', f0_mask.shape)

        if f0.size(1) % 4 != 0:
            pad_length = 4 - f0.size(1) % 4
            # print(f0.size(1), 'f0.size(1) % 4 != 0', pad_length) 
            f0 = F.pad(f0, pad=(0,0,0,pad_length))
            # print(f0.shape)
            f0_mask = F.pad(f0_mask, pad=(0,pad_length), value=1)
            # print(f0_mask)

        # convert int f0_lens into bool
        f0_mask = f0_mask.bool()

        # print('after pad mask (%4==0):', f0_mask)
        # print('after pad mask (%4==0):', f0_mask.shape)

        f0 = self.pitch_emb(f0.squeeze(-1))
        f0 = f0.transpose(1, 2)
        # f0 = f0.masked_fill(f0_mask.unsqueeze(1).expand_as(f0), 0.0)
        # print('after self.pitch_emb f0:', f0)
        # print('after self.pitch_emb f0:', f0.shape, torch.max(f0))
        h, subsampled_f0_mask = self.f0_encoder(f0, f0_mask) 
        # print('after self.f0_encoder f0:', h)
        # print('after self.f0_encoder f0:', h.shape)
        # print('f0.shape:', f0.shape, 'h.shape:', h.shape)
        f0_emb = h + self.res_conv(f0)
        f0_emb = f0_emb.masked_fill(subsampled_f0_mask.unsqueeze(1).expand_as(f0_emb), 0.0)
        f0_emb = f0_emb.transpose(1, 2)
        # print('f0_emb:', f0_emb)
        # print('f0_emb:', f0_emb.shape)
        ...
        if encoder_out.shape[1] != f0_emb.shape[1]:
            if encoder_out.shape[1] > f0_emb.shape[1]:
                ...
                f0_mask = F.pad(f0_mask, pad=(0,0,0,pad_length))
            else:
