...
from transformer_w2v import Supervisions, Transformer, encoder_padding_mask
from encoder_interface import EncoderInterface
from torch.nn.utils.rnn import pad_sequence
from transformer_w2v import generate_square_subsequent_mask, decoder_padding_mask, PositionalEncoding, TransformerDecoderLayer, LabelSmoothingLoss
class Conformer(nn.Module):
    ...
    def run_encoder(
        self, x: Tensor, x_lens: Tensor, is_training=False, supervisions: Optional[Supervisions] = None
        ...
        # mask = encoder_padding_mask(x.size(1) // 640, supervisions)
        # if mask is not None:
    ...
    def forward(
        self, x: torch.Tensor, x_lens: torch.testing, 
        y, sos_id,eos_id,
        is_training=False, supervision: Optional[Supervisions] = None
        ...
        encoder_memory, memory_key_padding_mask = self.run_encoder(x, x_lens, is_training=is_training, supervisions=supervision)
        x = self.ctc_output(encoder_memory)
        decoder_loss = self.decoder_forward(encoder_memory, memory_key_padding_mask, y, sos_id, eos_id)
        # return x, encoder_memory, memory_key_padding_mask, decoder_loss
        return x, decoder_loss
