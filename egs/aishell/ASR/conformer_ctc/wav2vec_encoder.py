...
class wav2vecEncoder(EncoderInterface):
    def __init__(
        self,
        model_dir: str,
        output_size: int = 768,
        ...
        if subsample_output:
            self.subsample = get_subsample_module(
                subsample_mode,
                input_size=model.cfg.encoder_embed_dim,
    ...
    def forward(
        ...
        # false and true, so ft is false, still in freezing mode, which means not finetune??
        ...
        # Start finetuning encoder after tensor([10001], device='cuda:0') updates!
        ...
        if enc_outputs["padding_mask"] is not None:
            ...
            olens = torch.IntTensor([xs_pad.shape[1]]).repeat(xs_pad.shape[0]).to(xs_pad.device)
