...
from train_phone_f0 import add_model_arguments, get_model, get_params
...
def f0ToPitch(f0):
    pitch = torch.log2(f0 / 27.5) * 12 + 21
    return torch.where(torch.isinf(pitch), torch.tensor(0.0), pitch)
...
def decode_one_batch(
    ...
    # add pitch embedding
    pitch_bins = torch.linspace(0, 120, 1200).to(pitch.device)
    pitch_emb = model.pitch_emb(torch.bucketize(pitch, pitch_bins))
